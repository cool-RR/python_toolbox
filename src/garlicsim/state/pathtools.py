...
def get_item_with_end_node(path, end_node, index):
    if index >= 0:
        return get_item_with_end_node_positive(path, end_node, index)
    else: # index < 0
        return get_item_with_end_node_negative(path, end_node, index)
    
def get_item_with_end_node_positive(path, end_node, index):
    result = path[index]
    if result.state.clock > end_node.state.clock:
        raise IndexError
    
def get_item_with_end_node_negative(path, end_node, index):
    return path._Path__get_item_negative(index, starting_at=end_node)


def get_node_by_monotonic_function_with_end_node(path, end_node,
                                                 function, value,
                                                 rounding="Closest"):
    
    both = path.get_node_by_monotonic_function(function, value, rounding="Both")
    ...
    return binarysearch.make_both_data_into_preferred_rounding(new_both,
                                                               function,
                                                               value,
                                                               rounding)
...
def get_node_by_clock_with_end_node(path, end_node, clock, rounding="Closest"):
    ...
    return get_node_by_monotonic_function_with_end_node(path, end_node,
                                                        get_clock, clock,
                                                        rounding=rounding)

def length_with_end_node(path, end_node):
    length = 0
